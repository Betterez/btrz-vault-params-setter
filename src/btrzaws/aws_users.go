...
func crossKeysWithUsers(awsSession *session.Session, iamService *iam.IAM, sourceFileName string) ([]*AwsUserInfo, error) {
	usersInfo, err := getUsersInformation(awsSession, iamService)
	if err != nil {
		return nil, err
	}
	existingKeys, err := getUsersKeysFromCSV(sourceFileName)
	if err != nil {
		return nil, err
	}
