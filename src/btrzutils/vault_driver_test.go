...
func loadTestValues() {
	sourceFile := "../../test-objects/test_values.json"
	jsonFile, err := os.Open(sourceFile)
	if err != nil {
		return
	}
	testJSON, err := simplejson.NewFromReader(jsonFile)
	if err != nil {
		return
	}
	defer jsonFile.Close()
	testVaultServer, err = testJSON.Get("vault-server").String()
	if err != nil {
		testVaultServer = ""
		return
	}
...
func TestConnectionFromJSONData(t *testing.T) {
	...
	driver, err := CreateVaultConnectionFromParameters(testVaultServer, token, 9000)
	if err != nil {
		t.Fatal(err)
	}
...
func TestJSONValues(t *testing.T) {
	...
	if resultData != "jarjar binxx" {
		t.Fatalf("return values equal to %s", resultData)
	}
}
...
func TestParametersLoading(t *testing.T) {
	...
	if params.Address != testVaultServer {
		t.Fatalf("expecting %s, got %s", testVaultServer, params.Address)
	}
}
...
func TestJSONKeys(t *testing.T) {
	...
	if results != expected {
		t.Fatalf("result = %s, should be %s", results, expected)
	}
}
...
func TestSimpleJSON(t *testing.T) {
	...
	allReposJSON.Set("repos", map[string]int{"repo1": 1})
	JSONStringData, err := allReposJSON.MarshalJSON()
	if err != nil {
		t.Fatal(err)
	}
	JSONStringString := string(JSONStringData)
	if JSONStringString != `{"repos":{"repo1":1}}` {
		t.Fatal("json values error", JSONStringString)
	}
}
...
func TestSetValue(t *testing.T) {
	...
	connection, err := CreateVaultConnectionFromParameters(testVaultServer, token, 9000)
	if err != nil {
		t.Fatal(err)
	}
